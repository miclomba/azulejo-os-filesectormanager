...
/********************************* def beg main *******************************
   ...
   Data Environment
	...
	Local Variables
		...
		int inputCount
		:= temporary integer for matching input read
...
int main(int _argc, char *_argv[])
{
	...
	if (_argc > 2 && atoi(_argv[2]) == 1)
	{
		...
		sprintf(dbfile, "./stubInput.txt");

		// open the stub input file, file should already exist
		driverHandle = fopen(dbfile, "r+");

		// read the driverHandle, storing number of characters read in inputCount
		/*inputCount =*/fread(driver, sizeof(char), MAX_INPUT, driverHandle);
	...
	else
	{
		...
		// read the driverHandle, store number of characters read in inputCount
		/*inputCount =*/fread(driver, sizeof(char), MAX_INPUT, driverHandle);
		...
		while (loop == True)
		{
			...
			case 'W':
				...
				if (digit > 0)
				{
					...
					if (digit > 0)
					{
						...
						// call to writeToFile
						success = writeToFile(fsm, inodeNumF, &buffer,
											  atoi(&driver[i]));
			...
			case 'T':
				...
				if (digit > 0)
				{
					...
					// move to the double indirect block
					fseek(fsm->diskHandle,
						  BLOCK_SIZE * (8 * index[2] + index[3]), SEEK_SET);
					...
					fwrite(block, sizeof(unsigned int), BLOCK_SIZE / 4,
						   fsm->diskHandle);
			...
			case 'M':
				...
				if (_argc > 1 && atoi(_argv[1]) == 1)
				{
					...
					mkfs(fsm, _DISK_SIZE, _BLOCK_SIZE, _INODE_SIZE,
						 _INODE_BLOCKS, _INODE_COUNT, 1);
